...
main: main.cu $(RECAP_BUILD_DIR)/ccamb.o $(RECAP_BUILD_DIR)/cic.o $(RECAP_BUILD_DIR)/initializer.o $(RECAP_BUILD_DIR)/io.o $(RECAP_BUILD_DIR)/power.o $(RECAP_BUILD_DIR)/greens.o $(RECAP_BUILD_DIR)/solver.o $(RECAP_BUILD_DIR)/params.o $(RECAP_BUILD_DIR)/timestepper.o $(RECAP_BUILD_DIR)/mmanager.o $(RECAP_BUILD_DIR)/ffts.o
	nvcc $^ -lcufft -lineinfo -Xptxas -v $(PY_LD_FLAGS) -lpython3.9 -Xcompiler -fPIC,-O3,-fopenmp,-g, -gencode arch=compute_60,code=sm_60 -o main
